...
//    runBlocking { //creates a coroutine that blocks the current thread(main thread) until the completion of the coroutine.
    ...
    suspendForDelay(2000) //block the current main thread and wait for the coroutine to finish(not a practical way to wait)

    println("Main program ends :  ${Thread.currentThread().name}")   //main thread
//    }
}

private suspend fun suspendForDelay(timeInMilli: Long){
    delay(timeInMilli)
}